      ...
      <el-card class="box-card" shadow="hover">
        <template #header>
          <div class="card-header">
            <span class="patent-span">专利详情</span>
          </div>
        ...
        >
          <el-form-item label="标题" prop="title">
            <el-input v-model="formPatent.title"></el-input>
          </el-form-item>
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="研究方向" prop="directionIds">
                <el-select
                  multiple
                  v-model="formPatent.directionIds"
                  @change="getReviewers"
                >
                  <el-option
...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file, fileList) {
      this.formPatent.url = file.response.data;
    ...
    handleSave() {
      ...
      request
        .post("/paper/save/", {
          id: this.formPatent.id,
          url: this.formPatent.url,
        })
        .then((res) => {
          console.log(res);
          if (res.status == 200) {
    ...
    getReviewers() {
      request
        .post("/user/identity", this.formPatent.directionIds)
        .then((res) => {
          // console.table(res.data);
    ...
    save() {
      this.$refs["formPatent"].validate((valid) => {
        console.log(valid);
        if (valid) {
          this.formPatent.uploaderId = this.getUserId();
          console.log(this.formPatent);

          if (this.formPatent.url === "") {
            this.$message({
              type: "error",
              message: "未选择文件",
            });
          ...
          request
            ...
            .then((res) => {
              ...
              if (res.data === "OK") {
                ...
                this.$refs["formPatent"].resetFields();
              } else {
                console.log(res);
                this.$message({
                  type: "error",
